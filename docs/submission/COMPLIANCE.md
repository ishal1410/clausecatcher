...
|---|---|---|---|---|---|
| 1 | "**Application URL:** Provide a link that allows interaction with your prototype." / RB: "**Application URL**: Required for interactive evaluation." Scoring floor: "1 - Poor Application \| ... Demo link is not available." | SUB §3, RB §3 | **FIXED 2026-09-20** | Live at **https://clausecatcher.onrender.com**. Verified by driving the hosted app: health ok (warm on the second call), WS `101` on the service origin and `403` on a foreign one, and one real contradiction flagged in 4.7 s. Note for the record: this finding was originally filed as Tier 1 "will cost the entry", which overstated it - re-reading https://lablab.ai/hackathon-rules on 2026-09-20, a missing Application URL is nowhere a rejection; it caps the Application-of-Technology axis at 1-2 of 5 ("Demo link is not available"), and the other three axes are untouched. | Deploy per `docs/DEPLOY.md` (Render Docker from `render.yaml`), set `ASSEMBLYAI_API_KEY` + `GEMINI_API_KEY` in Render env, drive one real alert, paste the URL. **Highest-value single action.** |
| 2 | "**Video Presentation:** A maximum 5-minute video in MP4 format." RB: "**Video and Slide Presentation**: MP4 and PDF formats are mandatory." Scoring floor: "Demo video is not available in video presentation." | SUB §2, RB §2, RB judging §3 | **FIXED 2026-09-20** | File exists and is valid: `docs/submission/demo.mp4`, ffprobe → h264 + aac, 1920x1080, 30 fps, **260.80 s (4:20.8)**, 25.6 MB (re-cut 2026-09-20, see finding 9). Uploaded to Vimeo at https://vimeo.com/1228498903 with privacy "Anyone"; verified reachable logged out (HTTP 200) and oEmbed reports 261 s. No placeholder remains. | Upload to Vimeo (privacy "Anyone"), paste clean `vimeo.com/<id>`. See also finding #9 — the runtime is a scoring problem. |
...
| 9 | RB judging, Presentation axis: "**2 - Limited** \| ... **Presentation video is less than 3 min.**" vs "**3 - Adequate** \| Effectively communicates the problem, solution, and value proposition **in less than 5 min**." | RB §3 judging table | **FIXED 2026-09-20** | `demo.mp4` is now **260.80 s = 4:20.8**, inside the 3:30–4:30 target and under the 5:00 limit. `tools/demo_video/append_closing.py` splices three narrated cards (market, why this is different, what's next) in front of the existing end card; the product footage is untouched. | Done. Re-runnable; the script asserts the result lands in 3:30–4:30. |
| 10 | RB Presentation axis, 4–5 scores require: "Explain market analysis and marketing revenue. Explain future goals & plans." / "shows project's strengths and uniqueness through **competitive analysis**." SUB Pro Tips: "Include Total Addressable Market (TAM) and Serviceable Addressable Market (SAM)." | RB §3, SUB §4 | **PARTIAL** | `LABLAB_SUBMISSION.md` long description has a competitor line ("Most call-review tools analyze recordings after the call ends...") and a pricing hypothesis, but **no TAM/SAM figures**. The deck (10 pages) has no market slide either. | **FIXED 2026-09-20**: the video's new market card carries the sizing — published 2026 conversation-intelligence estimates ($1.6B–$32B, spread shown deliberately), 1.59M US wholesale + manufacturing sales reps (BLS 2025), $0.6B–$1.1B at a $30–60 per-seat hypothesis, beachhead 292k technical/scientific reps — plus a competitive card and a roadmap card. The deck now carries them too: new slide 10, "The category, and the slice we can name", cloned from slide 9's stat layout with the same four figures character-for-character, so deck and video cannot drift apart. |

### Tier 3 — verified compliant, no action

| # | Rule (quoted) | Source | Status | Evidence |
|---|---|---|---|---|
| 11 | "Dates: Sep 1–30, 2026" / "Online Tuesday, September 1 2026 - 3:00 PM Coordinated Universal Time" | EVT | **PASS** | `git log --reverse --date=iso`: first commit `8442189` **2026-09-14 18:54:12 -0400** (= 2026-09-14 22:54 UTC), last `a4077c4` 2026-09-20. All 14 commits fall inside Sep 1–30. **No spike history predates the window** — `spikes/` was committed Sep 16, inside the window. |
...
| 18 | "**Slide Presentation:** Summarize your project in a PDF format slide presentation." | SUB §2 | **PASS** | `ClauseCatcher.pdf` — `%PDF-1.7`, 11 pages, 540 KB. |
| 19 | "**Short Description:** A concise summary (up to 255 characters)" | SUB §1 | **PASS** | 226 characters (counted in `LABLAB_SUBMISSION.md`). |
| 20 | "**Long Description:** Detailed write-up (at least 100 words)" | SUB §1 | **PASS** | ~945 words. |
...
2. **Upload `demo.mp4` to Vimeo and paste the link.** The re-export is done (4:20.8); only the upload is left.
