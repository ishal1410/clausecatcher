/**
 * dsp.js — pure audio-math helpers shared by audio-worklet.js (mic capture,
 * runs in AudioWorkletGlobalScope) and app.js (playback, runs on window).
 * No DOM, no network, no `window`/`AudioWorkletGlobalScope` globals used
 * here — that's what makes these testable from dev-check.html without a
 * live mic or server.
 */
...
export function base64ToInt16Array(b64) {
...
export function nextScheduleTime(nextStartTime, currentTime) {
 ...
 */
export function simulateSchedule(durationsSec, currentTime = 0) {
